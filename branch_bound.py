...
def branch_and_bound(item_count, capacity, weight_cost):
    ...
    :param item_count: number of existing items
    ...
    a_node = Node(0, [], 0.0, 0.0, calculate_bound(best_so_far, item_count, capacity, weight_cost, ratios))
    ...
    while len(priority_queue) > 0:
        ...
        if curr_node.bound > best_so_far.cost:
            ...
            next_not_added.bound = calculate_bound(next_not_added, item_count, capacity, weight_cost, ratios)
            if next_not_added.bound > best_so_far.cost:
                priority_queue.push(next_not_added)

    best_combination = [0] * item_count
    for wc in best_so_far.selected_items:
        best_combination[wc] = 1
    print(int(best_so_far.cost), best_combination)
